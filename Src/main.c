...
/* USER CODE BEGIN PV */
osPoolId MaximumValueHandle;
osPoolId MinimumValueHandle;
/* USER CODE END PV */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint32_t     SensorResultDMA[DMA_BUFF_SIZE];
...
int main(void)
{
  ...
  MX_CRC_Init();
  /* USER CODE BEGIN 2 */
  DopplerPinUp(POWER_LED_GPIO_Port,POWER_LED_Pin);
  /* USER CODE END 2 */
...
/* USER CODE BEGIN 4 */
void HardFedDog(uint8_t* FedDog)
{
  if(*FedDog == 0)
  {
    *FedDog = 1;
